# Limpieza y Transformación de Datos Deportivos - LaLiga ⚽🐍

Este repositorio contiene el proyecto práctico desarrollado en el marco de estudio del **Diplomado en Analítica Avanzada de Datos Deportivos con Python**. El objetivo principal es aplicar técnicas de ingeniería y manipulación de datos sobre registros de rendimiento futbolístico.

## 📋 Descripción del Proyecto

El proyecto se centra en la adquisición, tratamiento preliminar y segmentación de métricas de partidos y jugadores (utilizando datos de la temporada de LaLiga). A través de un cuaderno de Jupyter (`.ipynb`).

## 🚀 Contenido y Características Principales

El código está estructurado en tres etapas clave del procesamiento de datos:

1. **Limpieza y Transformación de Datos:**
   - Importación de dataframes estructurados a partir de archivos masivos en formato CSV (`fbref_laliga21.csv`).
   - Normalización de formatos, manejo de valores nulos o ausentes, y renombrado de columnas.

3. **Filtrado de Datos:**
   - Segmentación utilizando condiciones lógicas en Pandas.

4. **Estructuras de Control Bucles y Condicionales:**
   - Implementación de bucles `for` combinados con sentencias condicionales `if-elif-else` para la automatización de clasificaciones.
   - Creación de métricas calculadas personalizadas iterando sobre los registros.
